...
class FavQuoteQueryDAOSpec extends PostgresInstance with Matchers {
  ...
  it should "return empty if the are no fav quotes for a user id" in {
    val resultFirstUser: Future[Seq[QuotesQuery]] = favQuoteQueryDAO.listFavQuotes(userUuid)
  ...
  it should "create a new fav record if the record with same user id is not present" in {
    val response: Future[FavQuoteQuery] = favQuoteQueryDAO.modifyFavQuote(userUuid, "CSV101")
    response.map { result =>
  ...
  it should "return list of quotes for a user id" in {
    favQuoteQueryDAO.modifyFavQuote(userUuid, "CSV102")
    val response: Future[Seq[QuotesQuery]] = favQuoteQueryDAO.listFavQuotes(userUuid)
  ...
  it should "modify the favTag boolean if the existing record is requested" in {
    val response: Future[FavQuoteQuery] = favQuoteQueryDAO.modifyFavQuote(userUuid, "CSV102")
    response.map { result =>
  ...
  it should "return the cached fav quotes from the database" in {
    val response: Future[Seq[QuotesQuery]] = favQuoteQueryDAO.listCachedFavQuotes(userUuid)
    response.map { favQuotes =>
      favQuotes.length shouldBe 1
    }
  }
